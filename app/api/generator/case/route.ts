...
export async function POST(req: Request) { // add
  ...
  try {
    ...
    const created = await (prisma as any).testCase.create({
      data: { suiteId, title, stepsJson: steps, expected, priority, order: order ?? 0 }
    });
    
    // Mark suite as dirty after creating a case
    await (prisma as any).testSuite.update({ where: { id: suiteId }, data: { dirty: true } });
...
export async function PATCH(req: Request) { // edit
  ...
  try {
    ...
    const found = await (prisma as any).testCase.findUnique({ where: { id } });
    if (!found) return NextResponse.json({ ok:false, error:"NOT_FOUND" }, { status:404 });
    const suite = await (prisma as any).testSuite.findFirst({ where: { id: found.suiteId, userId } });
    if (!suite) return NextResponse.json({ ok:false, error:"FORBIDDEN" }, { status:403 });

    const updated = await (prisma as any).testCase.update({
      where: { id },
      data: {
        title: title ?? found.title,
        stepsJson: Array.isArray(steps) ? steps : (found.stepsJson as any),
    ...
    // Mark suite as dirty after updating a case
    await (prisma as any).testSuite.update({ where: { id: found.suiteId }, data: { dirty: true } });
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    await (prisma as any).testCase.delete({ where: { id } });
    
    // Mark suite as dirty after deleting a case
    await (prisma as any).testSuite.update({ where: { id: found.suiteId }, data: { dirty: true } });
